...
def calculate_tz(R_Noise_He_new, t_S_Noise_He_opt, p_w, p_si_noise, phi_max):
    ...
    def func_z_norm(tz, R, tx, ty, x3d, x2d):
        t = np.array([[tx, ty, tz.item()]], dtype=float).transpose()
        xc = np.dot(R, x3d) + t
        xc = xc.astype('float')
    ...
    def con(tz):
        t = np.array([[t_S_Noise_He_opt[0], t_S_Noise_He_opt[1], tz.item()]],dtype=float).transpose()
    ...
    result_tz_c2 = minimize(func_z_norm, 0.0,
                            args=(R_Noise_He_new, t_S_Noise_He_opt[0], t_S_Noise_He_opt[1], p_w, p_si_noise),
